...
import br.com.app.controller.hardware.DisplaysController;
...
@WebMvcTest(DisplaysController.class)
public class DisplaysControllerTest {
	...
	public static void setup() {
		...
		json = loadJson.loadDisplays();
	}
	
	@Test
	public void deveSalvarDisplays() throws Exception {
		
		String coletaResultadoJson = new ColetaResultadoTestDataBuilder().setJson(json).builderForJson();
		
		mockMvc.perform(post("/api/displays")
		           .contentType(MediaType.APPLICATION_JSON)
		           .content(coletaResultadoJson) 
		           .accept(MediaType.APPLICATION_JSON)).andExpect(status().isCreated());
		
		verify(displaysService).save(Mockito.any(ColetaResultado.class));
	}
}
